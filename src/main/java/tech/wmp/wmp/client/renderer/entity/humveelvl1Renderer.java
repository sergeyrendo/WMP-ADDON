...
import tech.wmp.wmp.client.model.humveelvl1Model;
...
public class humveelvl1Renderer extends GeoEntityRenderer<humveelvl1Entity> {

    public humveelvl1Renderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new humveelvl1Model());
        // Убираем дополнительный светящийся слой
        // this.addRenderLayer(new Lav150Layer(this));
    }

    @Override
    public RenderType getRenderType(humveelvl1Entity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(getTextureLocation(animatable));
    }

    @Override
    public void preRender(PoseStack poseStack, humveelvl1Entity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
    ...
    @Override
    public void render(humveelvl1Entity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, humveelvl1Entity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
    ...
    @Override
    protected float getDeathMaxRotation(humveelvl1Entity entityLivingBaseIn) {
        return 0.0F;
    }
}
