...
import tech.wmp.wmp.client.model.humveelvl2Model;
...
public class humveelvl2Renderer extends GeoEntityRenderer<humveelvl2Entity> {

    public humveelvl2Renderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new humveelvl2Model());
        // Убираем дополнительный светящийся слой
        // this.addRenderLayer(new Lav150Layer(this));
    }

    @Override
    public RenderType getRenderType(humveelvl2Entity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(getTextureLocation(animatable));
    }

    @Override
    public void preRender(PoseStack poseStack, humveelvl2Entity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
    ...
    @Override
    public void render(humveelvl2Entity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, humveelvl2Entity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
    ...
    @Override
    protected float getDeathMaxRotation(humveelvl2Entity entityLivingBaseIn) {
        return 0.0F;
    }
}
