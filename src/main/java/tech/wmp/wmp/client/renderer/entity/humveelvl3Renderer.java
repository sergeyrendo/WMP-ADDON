...
import tech.wmp.wmp.client.model.humveelvl3Model;
...
public class humveelvl3Renderer extends GeoEntityRenderer<humveelvl3Entity> {
    ...
    public humveelvl3Renderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new humveelvl3Model());
        // Убираем дополнительный светящийся слой
        // this.addRenderLayer(new Lav150Layer(this));
    }

    @Override
    public RenderType getRenderType(humveelvl3Entity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(getTextureLocation(animatable));
    }

    @Override
    public void preRender(PoseStack poseStack, humveelvl3Entity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green,
    ...
    @Override
    public void render(humveelvl3Entity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, humveelvl3Entity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
    ...
    @Override
    protected float getDeathMaxRotation(humveelvl3Entity entityLivingBaseIn) {
        return 0.0F;
    }
}
